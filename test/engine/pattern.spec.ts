import { execPattern, transform, Context, Request } from '../../src/engine';
...
describe('Pattern', () => {
  ...
  describe('RegEx Syntax', () => {
    it('should match a part of sentence', async () => {
      const pattern = transform('like', new Request(), new Context(), false);
      assert.instanceOf(pattern, RegExp);
      assert.isTrue(pattern.test(INPUT_TEXT));
    });

    it('should match a named variable', async () => {
      const pattern = transform('/^I would (?<verb>.+) to/', new Request(), new Context(), false);
      assert.isTrue(pattern.test(INPUT_TEXT));
    ...
    it('should captures intent and entities', async () => {
      const pattern = transform('/^I (.*) to (?<verb>.+) (\\d+) (?<what>.*)/', new Request(), new Context(), false);
      assert.isTrue(pattern.test(INPUT_TEXT));
      const captures = execPattern(INPUT_TEXT, pattern);
      assert.deepEqual(captures, {
        $1: 'would like',
  ...
  describe('BotScript Syntax', () => {
    it('should match exact same sentence', async () => {
      const pattern = transform(INPUT_TEXT, new Request(), new Context(), false);
      assert.isTrue(pattern.test(INPUT_TEXT));
    });

    it('should not match part of sentence', async () => {
      const pattern = transform('wou', new Request(), new Context(), false);
      assert.isFalse(pattern.test(INPUT_TEXT));
    });

    it('should match star wildcard', async () => {
      const pattern = transform('I would like *', new Request(), new Context(), false);
      assert.isTrue(pattern.test(INPUT_TEXT));
    ...
    it('should match number wildcard', async () => {
      const pattern = transform('buy # tickets', new Request(), new Context(), false);
      assert.isTrue(pattern.test(INPUT_TEXT));
